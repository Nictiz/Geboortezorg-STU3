    ...
    <xsl:template match="concept" mode="addElementForConcept">
        ...
        <xsl:variable name="fullPath">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:value-of select="concat($path,'.',@shortName)"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
<!--        <xsl:variable name="fhir-datatype-profile">
            <xsl:choose>
                <xsl:when test="$ada-datatype=''">BackBoneElement</xsl:when>
                <xsl:when test="$ada-datatype='code'">CodeableConcept</xsl:when>
                <xsl:when test="$ada-datatype='identifier'">Identifier</xsl:when>
                <xsl:when test="$ada-datatype='decimal'">decimal</xsl:when>
                <xsl:when test="$ada-datatype='integer, count'">integer</xsl:when>
        ...
        <element>
            <id value="{$fullPath}"/>
            <path value="{$fullPath}"/>
            <min value="0"/> <!-- todo: for transacties ophalen via ad file -->
            <max value="*"/>
            <type>
